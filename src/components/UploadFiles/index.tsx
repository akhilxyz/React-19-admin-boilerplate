...
interface UploadFileProps {
  maxCount?: number // Upload file count
  value?: string // File URL string
  fileSize?: number // File size 5GB
  accept?: 'image/*' | 'video/*' | 'image/*,video/*' // Accepted file types
  width?: number // Width 120
  height?: number // Height 120
  title?: string // Title, click to upload
  ...
  function beforeUpload(file: File): Promise<File> {
    return new Promise((resolve, reject) => {
      ...
      if ((size / 1024 / 1024) > fileSize) {
        ...
        reject(new Error(`File size cannot exceed ${fileSize}MB!`))
      }
      // File type
      ...
      if (!flag) {
        ...
        reject(new Error('Uploaded file does not meet the required format!'))
      }
      resolve(file)
    })
  }
